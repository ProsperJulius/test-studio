...
**Running** follows Playwright's rules: text and names match case-insensitively as a substring unless `exact: true`; `getByRole` ignores elements hidden from assistive technology unless `includeHidden: true`; and locators are **strict**. A step fails straight away if its locator matches more than one element, telling you to add `.first()`, `.nth()` or be more specific. Test data works inside locators, and in the **On** box for older recorded steps: `getByRole('row', { name: 'Order {orderId}' })`. If a `{name}` has no value when the step runs, the step stops and says so rather than looking for the name itself.

**Components.** A framework such as PrimeNG or Angular Material wraps each control in an element of its own — `<p-button>` round a `<button>`, `<p-inputtext>` round an `<input>` — and a test ID put on the component lands on that wrapper. Steps look inside: clicking, typing, selecting and the enabled, disabled and field-value checks all use the one control the wrapper holds, because a click on a wrapper is never seen by the button in it and a wrapper has no value or disabled state of its own. Reading text is left alone: a step asking what an element says means the element it names. A wrapper with no size of its own still counts as shown while the control in it is on screen, and the same test ID on both a wrapper and the control inside it is read as one element rather than two. Clicking, typing, selecting, pressing Enter and the two composite steps all go through Playwright, which waits for the element to be visible, still, enabled and able to receive what is being done to it. Something that could not have been worked — a disabled button, one covered by another element, a field that cannot be typed into — is reported rather than passing quietly. The checks stay as they were: they already wait on the step's own clock, and they read an element in ways a single call does not, such as taking a field's value when it has no text.

**Frames and shadow DOM.** Applications often show part of themselves in a frame, and a component keeps its markup in a shadow root. Steps reach into both: finding an element, clicking, typing, selecting and the checks all work through them, and text checks read what is inside them too. A frame served from a different address than the page can be worked by anything Playwright resolves — a locator — but not by the page's own code, which the browser will not let read it; that affects steps with no locator and grid steps. **Verify text appears** reads frames from the same address only, for the same reason.
...
- **Editing:** double-click the cell, type, and press Enter. The Type and Press Enter steps are tied to the same cell, and a single **Type and press Enter** step does both. A Type step on a cell that is not being edited opens it with a double-click by itself.
