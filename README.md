...
**Shadow DOM.** A component that keeps its markup in a shadow root is reached like any other: steps look inside open shadow roots, as Playwright does when it finds the element. The one thing that does not is **Verify text appears** and **Verify text is not shown**, which read the page's own text and cannot see inside a shadow root. Check wording in one of those components with **Verify element text** and a locator instead.
